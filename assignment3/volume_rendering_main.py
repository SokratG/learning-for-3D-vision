...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            plt.imsave(f'images/grid.png', xy_vis)

        # TODO (Q1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            xy_rays = vis_rays(ray_bundle, image_size)
            plt.imsave(f'images/rays.png', xy_rays)
        ...
        if cam_idx == 0 and file_prefix == '':
            render_points(filename=f'images/sample_points.png', points=ray_bundle.sample_points.view(1, -1, 3), device=device)
