...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        # print(deltas.shape)
        # T = torch.ones((deltas.shape[0], deltas.shape[1], 1), dtype=deltas.dtype, device=deltas.device)
        
        # T_i = T_{i-1} * e^{-density_{i-1}*dt}
        # print(rays_density[16000, :32])
    ...
    def _aggregate(
        ...
        feature = torch.sum(L, dim=1) # torch.sum(L) 
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            # print(feature.shape)
            # print(weights.shape)
            # print(feature.view(-1, n_pts, 3).shape)
...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        ...
        pts = origins.clone()
        # foreground_masks = torch.all(torch.isfinite(origins), dim=-1, keepdim=True)
        for _ in range(self.max_iters):
            sdf = implicit_fn(pts)
            pts = pts + directions * sdf

        mask_sdf = implicit_fn(pts)
        mask_hit = (mask_sdf <= self.mask_threshold) # & ~(mask_sdf > self.far) & ~(mask_sdf < self.near)
        return pts, mask_hit    
