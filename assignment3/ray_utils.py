...
def get_pixels_from_image(image_size: Tuple[int, int], camera: CamerasBase):
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Generate pixel coordinates from [0, W] in x and [0, H] in y
    # TODO (Q1.3): Convert to the range [-1, 1] in both x and y
    x, y = torch.linspace(-1, 1, steps=W), torch.linspace(-1, 1, steps=H)
...
def get_random_pixels_from_image(n_pixels, image_size: Tuple[int, int], camera: CamerasBase):
    W, H = image_size
    xy_grid = get_pixels_from_image(image_size, camera)
    
    # TODO (Q2.1): Random subsampling of pixel coordinaters
    unif = torch.ones(xy_grid.shape[0])
    sub_idx = torch.multinomial(unif, num_samples=xy_grid.shape[0], replacement=False)
    xy_grid_sub = xy_grid[sub_idx].to(camera.device)
...
def get_rays_from_pixels(xy_grid: torch.Tensor, image_size: Tuple[int, int], camera: CamerasBase):
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Map pixels to points on the image plane at Z=1
    ndc_pts = xy_grid.to(camera.device).clone()
