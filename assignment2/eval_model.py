...
from render import render_voxels, render_pc, render_pmesh
from utils import get_device

def get_args_parser():
    parser = argparse.ArgumentParser('Singleto3D', add_help=False)
    parser.add_argument('--arch', default='resnet18', type=str)
    parser.add_argument('--vis_freq', default=25, type=int)
...
def fail_result(thresholds):
    metrics = {}
    for t in thresholds:
        metrics["Precision@%f" % t] = 0
        metrics["Recall@%f" % t] = 0
        metrics["F1@%f" % t] = 0
    return metrics
...
def evaluate_model(args):
    ...
    if args.type == 'vox':
        f1_05_threshold = 55.0
    elif args.type == 'point':
        f1_05_threshold = 80.0
    else:
        f1_05_threshold = 80.0
    ...
    for step in range(start_iter, max_iter):
        ...
        if f1_05 >= f1_05_threshold:
            if (step % args.vis_freq) == 0:
                if args.type == 'vox': # [1, 32, 32, 32]
                    render_voxels(predictions.squeeze(1), f'images/model_voxel_pred_{step}.gif')
                    render_voxels(feed_dict['voxels'].squeeze(1).to(args.device), f'images/model_voxel_gt_{step}.gif')
                if args.type == 'point':
                    pointclouds_tgt = sample_points_from_meshes(mesh_gt, n_pts)
                    render_pc(predictions, f'images/model_pc_pred_{step}.gif')
                    render_pc(pointclouds_tgt.to(args.device), f'images/model_pc_gt_{step}.gif')
                if args.type == 'mesh':
                    render_pmesh(predictions, f'images/model_mesh_pred_{step}.gif')
                    render_pmesh(mesh_gt.to(args.device), f'images/model_mesh_gt_{step}.gif')
...
if __name__ == '__main__':
    #> py eval_model.py --type mesh --load_feat --load_checkpoint
